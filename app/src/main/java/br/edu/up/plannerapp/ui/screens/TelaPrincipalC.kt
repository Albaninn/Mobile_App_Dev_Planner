...
@Composable
fun TelaPrincipalC(state: DrawerState){
    Scaffold(
        topBar = { PlannerTopBar(state) },
        content = { p -> ConteudoPaginaPrincipalC(Modifier.padding(p)) },
...
@Composable
fun ConteudoPaginaPrincipalC(padding: Modifier){
    Column (
        modifier = Modifier.fillMaxSize(),
        verticalArrangement = Arrangement.Center,
        horizontalAlignment = Alignment.CenterHorizontally
    ) {
        Text(text = "Página principal C", fontSize = 50.sp)
    }
}
